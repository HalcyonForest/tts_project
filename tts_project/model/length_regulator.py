...
class LengthRegulator(nn.Module):
    ...
    def forward(self, x, alpha=1.0, target=None, mel_max_length=None):
        ...
        if target:
            H_mel = self.LR(x, duration, mel_max_length)
        else:
            duration = (
                duration * alpha + 0.5
                ).int()
            H_mel = self.LR(x, duration)
            duration = torch.stack([torch.Tensor([i+1 for i in range(H_mel.size(1))])]).long().to(self.model_config.device)
        
        return H_mel, duration
